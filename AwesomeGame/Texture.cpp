...
bool Texture::Load(const std::string& FileName) {
	...
	if (!LoadedSurface) {
		Debug::Log(EMessageType::ERROR, "Texture", "Load", __TIMESTAMP__, __FILE__, __LINE__, "Failed to load " + FileName + ". SDL Error: " + SDL_GetError());
	}
	else {
		if (SDL_SetColorKey(LoadedSurface, SDL_TRUE, SDL_MapRGB(LoadedSurface->format, 0, 0xFF, 0xFF)) < 0) {
			Debug::Log(EMessageType::ERROR, "Texture", "Load", __TIMESTAMP__, __FILE__, __LINE__, "Failed to set color key for " + FileName + ". SDL Error: " + SDL_GetError());
		}

		NewSDLTexture = SDL_CreateTextureFromSurface(SDLRenderer, LoadedSurface);
		if (!NewSDLTexture) {
			Debug::Log(EMessageType::ERROR, "Texture", "Load", __TIMESTAMP__, __FILE__, __LINE__, "Failed to create texture from " + FileName + ". SDL Error: " + SDL_GetError());
		}
		else {
...
void Texture::Draw(const int x, const int y, const float scale, 
